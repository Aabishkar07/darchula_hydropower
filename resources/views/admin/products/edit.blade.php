...
@section('body')
    <div class="flex gap-4">
        ...
        <div class="text-xl font-bold">Edit Projects</div>
    </div>
        ...
        <form action="{{ route('admin.products.update', $product->id) }}" method="POST" class="space-y-6"
            ...
            <div class="p-6 mt-3">
                <div class="flex flex-col ">
                    ...
                    <div class="mt-4">
                        ...
                        <div class="w-full max-w-md mt-2">
                            <select id="options" name="service"
                                class="block w-full px-4 py-2 text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:border-blue-500 ">
                                <option value="" disabled {{ old('options') === null ? 'selected' : '' }}>Select
                                    @foreach ($services as $service)
                                        an option
                                </option>
                                <option value="{{ $service->id }}" {{ old('service') == $service->id || $product->service_id==$service->id  ? 'selected' : '' }}>
                                    {{ $service->title }}
                                </option>
                                @endforeach
                    ...
                    <div class="mt-3">
                        <label class='text-sm font-semibold'>Projects Image</label>
                                ...
                                @foreach (getCategories(0) as $category)
                                    <div class="flex items-center">
                                        <input onclick="checkOnlyOne(this)"1 value={{ $category->id }} name="category"
                                            ...
                                            {{ old('category') == $category->id || $category->id == $product->category_id ? 'checked' : '' }}
                                            ...
                                            @foreach (getCategories($category->id) as $subcategory)
                                                <div class="flex items-center">
                                                    ...
                                                    <input type="checkbox" name="category" value="{{ $subcategory->id }}"
                                                        {{ old('category') == $subcategory->id || $subcategory->id == $product->category_id ? 'checked' : '' }}
                                                        class="w-4 h-4 border-gray-300 rounded text-secondary focus:ring-secondary"
