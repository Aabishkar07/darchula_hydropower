...
@section('body')
    ...
    <div class="flex items-center">
        <div class="flex-1">
            <h2 class="w-full text-2xl font-bold text-secondary ">Faqs</h2>
        </div>
        <div class="text-right "> <a href="{{ route('admin.faqs.create') }} "
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        @foreach ($faqs as $key => $faq)
                            <tbody class="bg-white divide-y divide-gray-200 ">
                                <tr>
                                    <td class="px-5 py-3 ">
                                        <p class="text-gray-900 ">{{ $faq->title }}</p>
                                    </td>
                                    <td class="px-5 py-3 ">
                                        <p class="text-gray-900 ">{{ $faq->description }}</p>
                                    </td>
                                    {{-- <td class="px-5 py-3 ">
                                        <p class="text-gray-900 ">{{ $faq->faqcategory->categoryname ?? '-' }}</p>
                                    </td> --}}
                                    ...
                                    <td>
                                        <div class="flex items-center p-2">
                                            ...
                                            <form method="POST" action="{{ route('admin.faqs.destroy', $faq->id) }}"
                                                ...
                                                @csrf
                                                @method('delete')
                                                <button type="button" onclick="deleteItem({{ $faq->id }})"
