                    ...
                    <table class="divide-y divide-gray-200">
                        ...
                        @foreach ($galleries as $key => $gallery)
                            <tbody class="bg-white divide-y divide-gray-200 ">
                                <tr>
                                    {{-- <td class="px-5 py-3 ">
                                        <p class="text-gray-900 ">{{ $gallery->title }}</p>
                                    </td> --}}


                                    <td class="p-2" style="width: 100px;">
                                        <img class="w-full h-full " src="{{ asset('uploads/' . $gallery->image) }}"
                                            alt="Card" style="width: 70px;">
                                    </td>

                                    <td class="w-48 px-5 py-3 text-sm">
                                        <p class="text-gray-900 ">
                                            {{ $gallery->created_at->format('jS M Y') }}
                                        </p>
                                    </td>
                                    ...
                                    <td>
                                        <div class="flex items-center p-2">
                                            ...
                                            <form method="POST" action="{{ route('admin.gallery.destroy', $gallery->id) }}"
                                                ...
                                                @csrf
                                                @method('delete')
                                                <button type="button" onclick="deleteItem({{ $gallery->id }})"
